...
- **Scroll animation wait**: use `waitForScrollAnimations(page)` from `tests/utils/timing.js` — polls DOM until every in-viewport `[data-animate]` element has computed opacity 1, capturing both the `.is-visible` class addition AND the 400ms opacity transition completion (class-only polling let axe-core sample mid-transition colors on WebKit and flake the color-contrast check). Short-circuits instantly under reduced motion (no-op — CSS sets opacity:1 unconditionally there anyway). Filter-hidden cards (`.project-card--hidden` → `visibility: hidden; position: absolute`) are skipped via explicit class check mirroring `js/main.js:595`; the rect's `r.width > 0` guard remains as defense-in-depth for any future zero-width `[data-animate]` element. Used in outer `beforeEach` of filter, modal, and form axe-scan suites. Replaces `fp.waitForScrollAnimations()` / `mp.waitForScrollAnimations()` POM methods (700ms fixed timeout) for suite-level setup

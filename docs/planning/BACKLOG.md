...
- [ ] `LINK_HREF` in `scripts/check-archived-links.js` matches a bare `superpowers/` substring instead of the fully-qualified `docs/superpowers/` — its pattern `/\]\([^)]*superpowers\//` diverges from the sibling `LINK_LABEL` (`/\[docs\/superpowers\/(?:specs|plans)\//`) and the `FORBIDDEN = 'docs/superpowers/'` constant, both of which anchor on the full path. No archived doc false-positives today, but a future archived plan that markdown-links an external href whose path contains `superpowers/` (e.g. `https://github.com/obra/superpowers/…`) would be flagged and break the pre-commit hook + CI `lint` job until the file is allowlisted. This also contradicts the CLAUDE.md "Archived-Doc Link Hygiene" claim that "external-repo refs are intentionally NOT flagged". Fix (corrected 2026-06-18): exclude external URLs with a negative lookahead — `/\]\((?!https?:)[^)]*superpowers\//` — which still flags both *local* dead forms (`](docs/superpowers/…)` and the relative `](../superpowers/…)`). Do **not** simply anchor to `docs\/superpowers\/`: that drops the relative `../superpowers/` href form (no `docs/` prefix), which actually occurred at `2026-06-07_backlog-restructure.md:280` pre-sweep — verified by test (shipped flags external ✗; `docs/superpowers/`-anchor misses relative ✗; lookahead flags both local, excludes external ✓). (PR #77 code review, confidence 50, latent correctness)
- [ ] The CLAUDE.md "Archived-Doc Link Hygiene" first bullet understates the guard's scope — it says the guard "fails on any nav-pointer line still pointing at `docs/superpowers/`", but the guard also flags the `LINK_LABEL` and `LINK_HREF` forms on lines that are not nav-pointer lines. The "Detection covers three forms" bullet immediately after states the full scope, so the intro sentence is narrower than the code. Reword the intro to "fails on any line matching one of the three detection forms", or let the three-forms bullet own the scope. (PR #77 code review, confidence 50, doc consistency)

### From Archived-Doc Dead-Link Cleanup Group E (2026-06-17)
**Origin**: docs/archive/plans/2026-06-17_archived-doc-dead-links.md

- [ ] `check-archived-links.js` reads the working tree, not the git index — the sibling guards (`validate-backlog-paths.js`, `check-backlog-structure.js`) read staged content via `git show :path`, but the archived-link guard does a working-tree directory scan. In the normal pre-commit case the staged `docs/archive/` files are on disk so it agrees; a partial stage (`git add -p`) could let an unstaged dead-link edit pass or a staged fix be missed. Align it to read the index for cross-guard consistency if this ever bites. (Group E plan "Out of scope", confidence 35, consistency)
- [ ] Upstream superpowers skill templates still author at `docs/superpowers/` with hyphen dates — `writing-plans`/`brainstorming` (plugin cache, out of repo) emit `docs/superpowers/<plans|specs>/YYYY-MM-DD-<name>` paths, so every new plan/spec starts in the dead-link-prone form and must be hand-retargeted at archival. The in-repo guard + the "Archived-Doc Link Hygiene" CLAUDE.md convention catch it post-hoc, but the root cause recurs each task and can't be fixed in-repo. Tracked so the recurrence stays visible. (Group E task-completion EXTRACT, confidence 40, process/upstream)
...
- [ ] `checkDistPreflight()`/`assetExists()` bare `catch {}` swallows EACCES — both functions in `scripts/check-assets.js` treat any `fs` error as "not found" / "missing or empty", so a permission error (EACCES) on `dist/` or an asset is reported as missing rather than as a distinct access failure. Consistent with the existing `assetExists()` pattern and very low-likelihood on a dev checkout; narrowing would mean rethrowing when `err.code` is neither `ENOENT` nor `ENOTDIR`. (Group C code review, confidence 30, defensive edge)
- [ ] Cross-file non-ASCII escape-style inconsistency — `scripts/validate-backlog-paths.js` now emits a literal `→` (U+2192) in the violation output, whereas `scripts/check-assets.js` expresses all non-ASCII glyphs via numeric Unicode escapes (the check/cross marks, em dash). Consistent within each file's own style (validate-backlog already uses literal em dashes in comments); cosmetic cross-file nit only. (Group C code review, confidence 20, style consistency)
- [ ] `FORBIDDEN_ORIGIN_PATHS` block comment could note subtree discrimination — the comment in `scripts/validate-backlog-paths.js` explains that `docs/superpowers/` covers both `plans/` and `specs/`, but now that the fix-guidance distinguishes those subtrees (Edit G) the comment could add that the suggester routes them to `docs/archive/specs/` vs `docs/archive/plans/`. Comment is correct as-is, just not exhaustive about the new downstream behavior. (Group C final review, confidence 25, doc completeness)

### From PR #74 Code Review (2026-06-14)
**Origin**: docs/archive/plans/2026-06-12_test-infra-cleanup.md

- [x] `waitForScrollAnimations` JSDoc prose hardcodes literal threshold values — the JSDoc in `tests/utils/timing.js` still describes the IntersectionObserver config as "10%" and "-50px" in prose even though the implementation now reads the exported `SCROLL_OBSERVER_THRESHOLD` / `SCROLL_OBSERVER_ROOT_MARGIN_BOTTOM` constants. The values currently match (0.1 = 10%, 50 = 50px) so there's no defect, but the prose should reference the constants by name so a future change to either constant doesn't silently desync the docstring. The new `timing-guards.spec.js` contract test would catch a value change, not a stale-prose change. (PR #74 code review, confidence 50, doc clarity) **[DONE 2026-06-14 — JSDoc prose now names the constants instead of "10%"/"-50px"]**
- [x] `timing-guards.spec.js` comment says observer is constructed "after init" — the explanatory comment states the production IntersectionObserver "is constructed inside a double requestAnimationFrame after init", but it is actually constructed inside `initScrollAnimations()` (`js/main.js:568-570`), which runs *during* the DOMContentLoaded init phase, not after it. The test polling is correct regardless; only the comment wording slightly mischaracterizes the timing. (PR #74 code review, confidence 50, comment accuracy) **[DONE 2026-06-14 — reworded for accuracy: the original "after init" was correct (the double-rAF defers `new IntersectionObserver` to after the synchronous DOMContentLoaded handler returns; 568-570 is the rAF scheduling, the actual construction is js/main.js:589). Clarified to spell out scheduled-during-init / runs-after-sync-init rather than adopting the review's less-accurate "constructed during init" framing.]**
...
- [x] Stale `docs/superpowers/specs/` display text in the new archived plan — the Spec link in `docs/archive/plans/2026-06-10_backlog-drain-cleanup-week.md` shows label text `docs/superpowers/specs/2026-06-10_backlog-drain-cleanup-week-design.md` while its relative href `../specs/...` correctly resolves to `docs/archive/specs/`. Link works; only the visible label is stale. Same recurring pattern as the restructure-docs entry below; fold into the Group E "Archived-Doc Dead-Link Cleanup" pass. [possible-dup-of: Update stale `docs/superpowers/` cross-references inside the archived restructure docs] (PR #73 code review, confidence 50, doc lineage) **[DONE 2026-06-17 — Group E Task 4: label retargeted to `docs/archive/specs/`; the guard was hardened (`LINK_LABEL`) to catch this wrapped-pointer form the line-anchored check missed. PR pending]**
...
- [x] CLAUDE.md drift after the `check-backlog-structure` guard shipped — the Pre-commit hook bullet (line 112) still says the hook "conditionally runs `npm run validate-backlog` only" when it now runs both `validate-backlog` and `check-backlog-structure` (each with `|| exit 1`); the Build & Development Commands block (line 33) and the `scripts/` architecture line (line 60) omit `check-backlog-structure`/`check-backlog-structure.js`. All three sites sit inside `<!-- AUTO-MANAGED -->` regions, so route the fix through the auto-memory sync (`/auto-memory:sync`) rather than a hand-edit the next sync would clobber. (PR #72 code review, confidence 50, doc drift) **[DONE 2026-06-16, Group B: all 3 sites synced by 687940a]**
- [x] DONE.md no-loss claim undercounts — the Backlog Restructure entry (line 19) states "234 baseline ∪ 3 promotions = 237; zero dropped, zero extra", but the restructure also added 3 net-new 🟤 follow-up items (the "From Backlog Restructure (2026-06-08)" section), so the final open-item count is ~240 and "zero extra" is inaccurate. The migration was lossless for pre-existing items; only the summary's "zero extra" wording is off. Historical-log accuracy nit, no functional impact. (PR #72 code review, confidence 50, doc accuracy) **[WON'T-DO 2026-06-16, Group B: "zero extra" is scoped to migration parity (234+3=237); the 3 EXTRACT follow-ups are documented separately in the same entry — accurate as written]**

### From Backlog Restructure (2026-06-08)
**Origin**: docs/archive/plans/2026-06-07_backlog-restructure.md

- [x] **Update stale `docs/superpowers/` cross-references inside the archived restructure docs** — the archived plan (`docs/archive/plans/2026-06-07_backlog-restructure.md`) and design spec (`docs/archive/specs/2026-06-07_backlog-restructure-design.md`) still contain internal links, bash commands, and PR/commit-message templates pointing at the pre-archive `docs/superpowers/specs|plans/` paths. Dead links in historical docs, no functional impact (validator only checks BACKLOG `**Origin**` lines). Same recurring pattern noted in the PR #66 post-merge review for the test-stability plan. (restructure follow-up, confidence 35, doc lineage) **[DONE 2026-06-17 — Group E: the 9 navigational pointers in the restructure plan (header `**Spec:**`, `**Design spec**:` cross-ref, footer `Spec:`/`Pass 1:` lines) retargeted to `docs/archive/`. Per the approved design, historical bash commands (`git add docs/superpowers/…`, `ls …`) and the design spec's intentional refs (the validator denylist literal, the external `rating_bot` path) are preserved as execution record — the new guard enforces navigational pointers only, not command history. PR pending]**
...
- [x] CLAUDE.md "Shell Gotchas" — tighten `Triggered by` reference to inline the file path `.husky/pre-commit` rather than leaving it parenthetical-only in the cross-linked Pre-commit bullet. A reader who lands on the gotcha via a "shell"/"grep" search vector without reading the Commits subsection won't have a concrete example. (PR #70 Task 2 review, confidence 40, minor wording polish) **[WON'T-DO 2026-06-16, Group B: AUTO-MANAGED region (sync-owned); bullet already cites .husky/pre-commit]**
- [x] CLAUDE.md "Shell Gotchas" — replace "On a fresh repo or unstaged-file commit" with "On any commit that doesn't stage BACKLOG.md". The current wording leads with the edge case rather than the common case (any everyday commit where BACKLOG.md isn't part of the staged set would have triggered the bad-pattern bug). (PR #70 Task 2 review, confidence 50, accuracy) **[WON'T-DO 2026-06-16, Group B: AUTO-MANAGED region (sync-owned); current wording accurate]**
- [x] ROADMAP.md — annotate v1.5's "Completed 2026-03-21" with a "developed in parallel with v1.1" parenthetical, or restructure the phase-ladder to acknowledge that v1.5 commits (theme toggle, filter, scroll animations, 404 — all 2026-01-28) predate later v1.1 items (Bing Webmaster Tools, 2026-02-10). The current sequential framing is inherited from the original ROADMAP and the phase rewrite preserved rather than fixed it. (PR #70 Task 3 review, confidence 60, important structural drift) **[DONE 2026-06-16, Group B: parallel-development footnote added]**
- [x] ROADMAP.md — remove duplicate "Last Updated" field. Header bold `**Last Updated**: 2026-05-10` (line 3) and trailing italic `*Last updated: 2026-05-10*` (line 84) both maintained; sibling docs (DONE.md, WEEKLY.md) use only the header form. Doubles the chance of stale-date bug on next update. (PR #70 Task 3 review, confidence 70, low risk) **[DONE 2026-06-16, Group B: trailing footer removed; header field retained]**
- [x] ROADMAP.md — align Quality & Hardening cross-link display-text with DONE.md convention. Current `[docs/planning/WEEKLY.md](WEEKLY.md)` uses full-path display + abbreviated relative href, inconsistent with DONE.md's `[docs/archive/specs/<file>](../archive/specs/<file>)` (full path in both positions). Cosmetic; either `[WEEKLY.md](WEEKLY.md)` or `[docs/planning/WEEKLY.md](../planning/WEEKLY.md)` would be consistent. (PR #70 Task 3 review, confidence 50, polish) **[DONE 2026-06-16, Group B: bare-name cross-links]**
- [x] ROADMAP.md — document transition convention for the in-progress phase header. "🔧 In Progress, since 2026-04" will read oddly if not updated to "✅ Completed YYYY-MM-DD" when Quality & Hardening closes. Either inline a comment or note the next-maintainer step in a tracking file. (PR #70 Task 3 review, confidence 40, forward-looking) **[DONE 2026-06-16, Group B: HTML transition comment added]**
- [x] BACKLOG.md observability item — update parenthetical at line 948 from `actions/checkout@v4` → `actions/checkout@v6` to match the post-PR-#70 state. Behavioral observation (lint job uses default full clone) is unchanged. (PR #70 Task 4 final review, confidence 60, doc accuracy — already partially addressed in this same PR's CI Deadline & Docs follow-up) **[OBSOLETE 2026-06-16, Group B: target observability item pruned in Group A; no live actions/checkout@v4 remains]**

### From BACKLOG Validator Hardening (2026-05-07)
**Origin**: docs/archive/plans/2026-05-06_backlog-validator-hardening.md

- [x] Improve validator fix-guidance for spec-targeted violations — error message at `scripts/validate-backlog-paths.js:70` says "Replace the forbidden path with the equivalent docs/archive/plans/... path" but spec violations should target `docs/archive/specs/`. Detect whether the violation line points to a `plans/` or `specs/` subtree and emit appropriate guidance. (PR #69 review finding, confidence 30, nitpick) **[DONE 2026-06-14, Group C — Edit G: detects specs/ vs plans/ subtree, suggests docs/archive/specs|plans/ per violation; `[matched:]` substring preserved]**

### From PR #69 Post-Merge Review (2026-05-07)

- [x] Surface a `console.warn` when `readBacklog()` falls back to working-tree read — `scripts/validate-backlog-paths.js:35-46` documents in a code comment that a future sparse-checkout or blob-filter mode in CI would cause `git show` to fail and silently fall back to the working-tree read. Today this never triggers (lint job uses `actions/checkout@v6`), but the silent-degradation hazard exists purely as documentation. Emitting `console.warn('readBacklog: git show failed; using working tree')` on the inner-catch path would make the degradation observable in CI logs and align with the repo's pattern of explicit feedback (link checker LinkedIn skips, asset checker preflight). (PR #69 review observation, confidence 50, observability) **[DONE 2026-06-14, Group C — Edit F: console.warn on the inner-catch git-entirely-unavailable fallback]**

### From PR #68 Post-Merge Review (2026-05-05)
**Origin**: docs/archive/plans/2026-05-02_asset-checker-polish.md

- [x] Guard `checkDistPreflight()` against `dist` existing as a non-directory — `scripts/check-assets.js:138` calls `fs.readdirSync(distDir)` after a bare `fs.existsSync()` check. If a stray file named `dist` (no trailing slash) is present at repo root (e.g., from `touch dist` or a broken build tool), `readdirSync` throws `ENOTDIR` with a raw stack trace instead of the clean "run `npm run build` first" message the function was designed to emit. The pre-existing `assetExists()` (line 119) already wraps `realpathSync.native` in try/catch — apply the same defensive pattern here, or check `fs.statSync(distDir).isDirectory()` before reading. (confidence 50, low likelihood but breaks the contract of the function's docstring) **[DONE 2026-06-14, Group C — Edit B: readdirSync wrapped in try/catch (catches ENOENT+ENOTDIR); existsSync removed]**
- [x] Reconcile `checkDistPreflight()` JSDoc vs error message vocabulary — `scripts/check-assets.js:131-134` JSDoc says "Fast-fails when dist/ is missing or empty," but the printed error at line 140 says "dist/ missing or incomplete." The word "incomplete" in the message overstates what the preflight actually checks (it does NOT detect partial/stale `dist/` — that case falls through to the stale-hash hint). Either change the message to "missing or empty" to match the JSDoc, or add a note in the JSDoc explaining the broader user-facing wording. (confidence 50, vocabulary drift, no functional impact) **[DONE 2026-06-14, Group C — Edit C: error now "missing or empty" matching the JSDoc]**
- [x] Update `extractJsonRefs()` JSDoc to mention the non-object guard — `scripts/check-assets.js:73-76` JSDoc reads "Walks projects[*].screenshots[].src. Skips excluded refs." but the function body now also skips entries where `typeof project !== 'object' || project === null` (line 81, added in PR #68). Either expand the JSDoc to "...also skips non-object projects[*] entries" or accept the omission as a defensive-only detail. (confidence 50, doc/code drift) **[DONE 2026-06-14, Group C — Edit D: JSDoc now notes the non-object guard]**

### From Asset Checker Polish Code Review (2026-05-03)
**Origin**: docs/archive/plans/2026-05-02_asset-checker-polish.md

- [x] Restyle the stale-hash hint label so the tone matches "helpful nudge" — `scripts/check-assets.js:194` prints the `Hint:` label in `RED`, which reads as an error declaration rather than the nudge the spec intended. Preflight `dist/ missing or incomplete` correctly uses red because it is an error, but the stale-hash hint is recoverable diagnostic context. Consider plain text for the `Hint:` label, or a dim/yellow ANSI escape if a new color constant is acceptable. Subjective styling; no behavior impact. (confidence 40) **[DONE 2026-06-14, Group C — Edit E: Hint label recolored RED→YELLOW (new YELLOW constant)]**
...
- [x] Update stale `docs/superpowers/` cross-references in archived plan — `docs/archive/plans/2026-04-28_test-stability-investigations.md` lines 11 and 581-582 reference `docs/superpowers/specs/...` and `docs/superpowers/plans/...` paths that no longer exist (artifacts went to `docs/archive/specs/` and `docs/archive/plans/`). Pre-commit `validate-backlog-paths.js` only checks BACKLOG `**Origin**:` lines, so these slipped through. Dead links in archived doc, no functional impact. (confidence 35) **[DONE 2026-06-17 — Group E: header `**Spec:**` (line 11) + footer `Spec:`/`Plan:` (581-582) retargeted to `docs/archive/specs|plans/`. PR pending]**
